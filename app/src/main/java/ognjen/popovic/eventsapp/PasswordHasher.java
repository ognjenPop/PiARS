...
public final class PasswordHasher {
    ...
    public static boolean verifyPassword(String enteredPassword, String storedPasswordData) {
        if (enteredPassword == null || storedPasswordData == null) {
            return false;
        }
        ...
        try {
            ...
            if (salt.length != SALT_BYTES || storedHash.length == 0) {
                return false;
            }

            byte[] enteredHash = pbkdf2(
                    algorithmId,
                    enteredPasswordChars,
                    salt,
                    iterations,
                    storedHash.length
            );
    ...
    private static byte[] pbkdf2(String algorithmId, char[] password, byte[] salt,
                                 int iterations, int hashBytes)
        ...
        try {
            return SecretKeyFactory
                    .getInstance(getJavaAlgorithmName(algorithmId))
                    .generateSecret(spec)
                    .getEncoded();
        } finally {
            spec.clearPassword();
    ...
    private static String getJavaAlgorithmName(String algorithmId) throws NoSuchAlgorithmException {
        switch (algorithmId) {
            ...
            default:
                throw new NoSuchAlgorithmException("Unsupported algorithm: " + algorithmId);
        }
    }
}